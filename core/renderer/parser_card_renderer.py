...
from .parser_card_data import CommentEntry, GraphicsContent, ParseResult
...
class FontSet:
    ...
    text_font: FontInfo
    extra_font: FontInfo
    comment_name_font: FontInfo
    comment_text_font: FontInfo
...
@dataclass(eq=False, frozen=True, slots=True)
class CommentSectionData(SectionData):
...
class Renderer:
    ...
    # 转发内容配置
    REPOST_TOP_PADDING = 4
    REPOST_BOTTOM_PADDING = 12
    """转发内容内边距"""
    REPOST_SCALE = 0.88
    """转发缩放比例"""
    ...
    async def text(
        ...
        draw = ctx.draw
        for line in lines:
            draw.text((x, y), line, font=font.font, fill=fill)
            y += font.line_height
        return font.line_height * len(lines)

    async def _draw_emphasized_text(
        self,
        ctx: RenderContext,
        xy: tuple[int, int],
        lines: list[str],
        ...
        await self.text(ctx, (xy[0] + 1, xy[1]), lines, font, fill=fill)
        return await self.text(ctx, xy, lines, font, fill=fill)
    ...
    async def _calculate_sections(
        ...
        comment_section = self._calculate_comment_section(
            result.comments,
            content_width,
        )
    ...
    def _calculate_comment_section(
        self,
        comments: list[CommentEntry],
        content_width: int,
    ) -> CommentSectionData | None:
        if not comments:
            return None

        blocks: list[CommentBlock] = []
        for comment in comments:
            nickname = comment.nickname.strip()
            content = comment.content.strip()
            name_text = f"{nickname}：" if nickname else ""
            name_width = (
                self.fontset.comment_name_font.get_text_width(name_text)
                if name_text
                else 0
            )

            if not name_text and not content:
                continue

            first_line_prefix_width = min(name_width, max(content_width - 1, 1))
            first_line_available_width = max(content_width - first_line_prefix_width, 1)

            content_lines: list[str] = []
            if content:
                first_line = self._wrap_text(
                    content,
                    first_line_available_width,
                    self.fontset.comment_text_font,
                )
                if first_line:
                    consumed = len(first_line[0])
                    content_lines.append(first_line[0])
                    remaining_content = content[consumed:]
            ...
            height = len(content_lines) * self.fontset.comment_text_font.line_height
        ...
        total_height += self.COMMENT_BLOCK_GAP * (len(blocks) - 1)
        return CommentSectionData(height=total_height, comments=blocks)
    ...
    async def _calculate_repost_section(self, repost: ParseResult) -> RepostSectionData:
        ...
        return RepostSectionData(
            height=scaled_height + self.REPOST_TOP_PADDING + self.REPOST_BOTTOM_PADDING,
    ...
    async def _draw_sections(
        ...
        for section in sections:
            match section:
                ...
                case CommentSectionData() as comments:
                    await self._draw_comments(ctx, comments)
    ...
    async def _draw_comments(
        self,
        ctx: RenderContext,
        section: CommentSectionData,
    ) -> None:
        for index, block in enumerate(section.comments):
            line_y = ctx.y_pos

            if block.name_text:
                await self._draw_emphasized_text(
                    ctx,
                    (self.PADDING, line_y),
                    [block.name_text],
                    self.fontset.comment_name_font,
                    fill=self.TEXT_COLOR,
                )

            if block.content_lines:
                first_line = block.content_lines[0]
                if first_line:
                    await self.text(
                        ctx,
                        (self.PADDING + block.name_width, line_y),
                        [first_line],
                        self.fontset.comment_text_font,
                        fill=self.TEXT_COLOR,
                    )

                if len(block.content_lines) > 1:
                    remaining_lines = block.content_lines[1:]
                    await self.text(
                        ctx,
                        (
                            self.PADDING,
                            line_y + self.fontset.comment_text_font.line_height,
                        ),
                        remaining_lines,
                        self.fontset.comment_text_font,
                        fill=self.TEXT_COLOR,
            ...
            ctx.y_pos += block.height

            if index < len(section.comments) - 1:
                ctx.y_pos += self.COMMENT_BLOCK_GAP
    ...
    def _draw_repost(self, ctx: RenderContext, section: RepostSectionData) -> None:
        ...
        card_y = repost_y + self.REPOST_TOP_PADDING
